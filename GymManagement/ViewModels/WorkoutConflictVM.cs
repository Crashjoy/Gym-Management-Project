        ...
        public string DurationSummary
        {
            get
            {
                ...
                else
                {
                    ...
                    string duration = "";
                    if (d.Minutes > 0) //Showing the minutes if there are any
                    {
                        duration = d.Minutes.ToString() + " min";
                    }
                    if (d.Hours > 0) //Showing the hours if there are any
                    {
                        duration = d.Hours.ToString() + " hr" + (d.Hours > 1 ? "s" : "")
                            + (d.Minutes > 0 ? ", " + duration : ""); //Putting a ", " between hours and minutes if there are both
                    }
                    if (d.Days > 0) //Showing the days if there are any
                    {
                        duration = d.Days.ToString() + " day" + (d.Days > 1 ? "s" : "")
                            + (d.Hours > 0 || d.Minutes > 0 ? ", " + duration : ""); //Putting a ", " between days and hours/minutes if there are any
                    }
                    return duration;
                }
            }
